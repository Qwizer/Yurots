...
@section("title") Lojas dos Entregadores - Dashboard
@endsection
@section('content')
<style>
        ...
        <div class="card">
            <div class="card-body">
                <legend class="font-weight-semibold text-uppercase font-size-sm">
                    <i class="icon-address-book mr-2"></i> Lojas do Entregador
                </legend>
                <div class="form-group row form-group-feedback form-group-feedback-right">
                    @if(count($userRestaurants) === 0)
                    <div class="col-lg-9">
                        <p class="text-warning">{{ $user->name }} não está designado para fazer entregas em nenhum Loja.</p>
                    </div>
                    @else
                    <br>
                    <div class="col-lg-9">
                        <p><strong>{{ $user->name }}</strong> fazendo entregas para <strong>{{ $userRestaurants->count() }} </strong>.</p>
                ...
                <div class="col-lg-12">
                    <form action="{{ route('admin.updateDeliveryGuysRestaurants') }}" method="POST">
                        ...
                        <div class="text-right mb-4">
                            <button type="button" class="btn btn-primary btn-labeled btn-labeled-left btn-sm" id="checkAll" data-popup="tooltip" title="Clique Duplo para Marcar Todos" data-placement="left">
                            <b><i class="icon-check ml-1"></i></b>
                                Marcar Todos
                            </button>
                            <button type="button" class="btn btn-primary btn-labeled btn-labeled-left btn-sm" id="unCheckAll" data-popup="tooltip" title="Clique Duplo para Desmarcar Todos" data-placement="top">
