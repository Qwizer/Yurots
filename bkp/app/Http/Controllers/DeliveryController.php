                ...
                if ($user->delivery_pin == strtoupper($request->delivery_pin)) {
                    ...
                    if (config('settings.enableDeliveryGuyEarning') == 'true') {
                        ...
                        if ($deliveryUser->user) {
                            $commission = $order->delivery_charge;
                            $valorCash = number_format(($order->payable * $order->restaurant->cashback) / 100,2,'.','');
                            $deliveryUser->user->deposit($commission * 100, ['description' => 'Restaurante '.$order->restaurant->name.' entrega ' . $order->unique_order_id]);
                            $order->user->deposit($valorCash * 100, ['description' => 'Restaurante '.$order->restaurant->name.' Cashback do pedido ' . $order->unique_order_id]);
                            
                         //   $user = User::where('id', $deliveryUser->user->id)->first();
                            //$user->withdraw($commission * 100, ['description' => 'Pagamento pela entrega ' . $order->unique_order_id]);
