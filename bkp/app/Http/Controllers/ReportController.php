...
class ReportController extends Controller
{
    ...
    public function viewTopItems(Request $request)
    {
        ...
        if ($restaurant_id) {
            ...
            // Range 1 = This Week
            if ($range == '1') {
                $orders = Order::orderBy('orders.id', 'DESC')->join('restaurants', 'orders.restaurant_id', '=', 'restaurants.id')->where('orders.orderstatus_id', '5')->where('orders.restaurant_id', $restaurant_id)->whereBetween('orders.created_at', [
            ...
            // Range 2 = Last 7 Days
            if ($range == '2') {
                $orders = Order::orderBy('orders.id', 'DESC')->join('restaurants', 'orders.restaurant_id', '=', 'restaurants.id')->where('orders.orderstatus_id', '5')->where('orders.restaurant_id', $restaurant_id)->whereDate('orders.created_at', '>', Carbon::now()->subDays(7))->paginate(20);
            ...
            if ($range == '4') {
                $orders = Order::orderBy('orders.id', 'DESC')->join('restaurants', 'orders.restaurant_id', '=', 'restaurants.id')->where('orders.orderstatus_id', '5')->where('orders.restaurant_id', $restaurant_id)->whereDate('orders.created_at', '>', Carbon::now()->subDays(30))->paginate(20);
            ...
            // Range 5 = All time
            if ($range == '5') {
                $orders = Order::orderBy('orders.id', 'DESC')->join('restaurants', 'orders.restaurant_id', '=', 'restaurants.id')->where('orders.orderstatus_id', '5')->where('orders.restaurant_id', $restaurant_id)->paginate(20);
        ...
        if (!$restaurant_id) {
            // Range 1 = This Week
            if ($range == '1') {
                $orders = Order::orderBy('orders.id', 'DESC')->join('restaurants', 'orders.restaurant_id', '=', 'restaurants.id')->where('orders.orderstatus_id', '5')->whereBetween('orders.created_at', [
                    Carbon::now()->startOfWeek(),
                    Carbon::now()->endOfWeek(),
                ])->paginate(20);
            ...
            // Range 2 = Last 7 Days
            if ($range == '2') {
                $orders = Order::orderBy('orders.id', 'DESC')->join('restaurants', 'orders.restaurant_id', '=', 'restaurants.id')->where('orders.orderstatus_id', '5')->whereDate('orders.created_at', '>', Carbon::now()->subDays(7))->paginate(20);
            ...
            // Range 4 = Last 30 Days
            if ($range == '4') {
                $orders = Order::orderBy('orders.id', 'DESC')->join('restaurants', 'orders.restaurant_id', '=', 'restaurants.id')->where('orders.orderstatus_id', '5')->whereDate('orders.created_at', '>', Carbon::now()->subDays(30))->paginate(20);
                $top_items_completed = Order::where('orderstatus_id', '5')->whereDate('created_at', '>', Carbon::now()->subDays(30))->pluck('id');
            }
            // Range 5 = All time
            if ($range == '5') {
                $orders = Order::orderBy('orders.id', 'DESC')->join('restaurants', 'orders.restaurant_id', '=', 'restaurants.id')->where('orders.orderstatus_id', '5')->paginate(20);
